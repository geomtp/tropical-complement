...
\begin{align}
     \opn{RR}(X;D)\deq \int_{X}\opn{ch}(\mathcal{L}_{D})\opn{td}(X).
\end{align}
ここで、$\mathcal{L}_{D}$とは、$D$が定める
トロピカル直線束であり、
$\opn{ch}(\mathcal{L}_{D})\deq 
\sum_{i=0}^{\infty}\frac{c_1(\mathcal{L}_{D})^{i}}{i!}$
は、$\mathcal{L}_{D}$ の Chern 指標であり、
$\int_X$ はトレース写像である。
...
引き戻しとして得られると考えられる。
$X$ が boundaryless なとき、より一般の結果として
