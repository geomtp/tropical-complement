...
とくに、$\mathcal{L}$ を $D$ が定義する直線束とすると、
...
\end{proposition}
\begin{proof}
$D_1',D_2'$ をそれぞれ $\mathcal{L}(D_1),\mathcal{L}(D_2)$
を表現する $\opn{Div}(X)^{[0]}$ に属す因子とする。
得られた因子は、
\cite[\textsection 2.4]{demedrano2023chern}
でも言及している通り
それぞれ $D_1$, $D_2$ と rationally
equivalent である。このとき、
\cite[Corollary 3.37]{shaw2015tropical} より
$D_1.D_2=D_1'. D_2'$ である。
...
部分トロピカル多様体とする。このとき、包含写像
$\iota\colon D \to X$ に対して次が成り立つ。
\begin{align}
\label{equation-total-adjunction}
\iota^{*}c^{\mathrm{sm}}(X)=c^{\mathrm{sm}}(D)c(\iota^{*}\mathcal{L}_{D}), \\ 
\iota^{*}c^{\mathrm{sm}}_k(X)=c^{\mathrm{sm}}_k(D)+
c^{\mathrm{sm}}_{k-1}(D)c_1(\iota^{*}\mathcal{L}_{D})
\end{align}
ここで、$c(\iota^{*}\mathcal{L}_{D})\deq 1+c_1(\iota^{*}\mathcal{L}_{D})
=\iota^{*}(1+c_1(\mathcal{L}_{D}))$である。 
\end{conjecture}
...
Gysin 押し出しと射影公式から
\begin{align}
c^{\mathrm{sm}}(X)\cdot [D]=\iota_!c^{\mathrm{sm}}(D)
c(\mathcal{L}_{D}).
\end{align}
...
\begin{align}
\iota^{*}\opn{td}(X)=
\opn{td}(D)\frac{c_1(\iota^{*}\mathcal{L}_{D})}
{1-\opn{exp}(c_1(\iota^{*}\mathcal{L}_{D}))}.
\end{align}
これを式変形すると次のようになる。
\begin{align}
\opn{td}(D)
=\iota^{*}\left(
\frac{1-\opn{exp}(c_1(\mathcal{L}_{D}))}{
c_1(\mathcal{L}_{D})}\opn{td}(X)\right)
\end{align}
また、これに射影公式
を当てはめると
\begin{align}
\label{equation-grr-divisor-2}
\iota_!(\opn{td}(D))=(1-\opn{ch}(\mathcal{L}_{D}))\opn{td}(X).
\end{align}
...
そのため射影公式と\eqref{equation-grr-divisor-2}より、
\begin{align}
\iota_!(\opn{ch}(\iota^{*}(\mathcal{L}_{D'}))\opn{td}(D))
=\opn{ch}(\mathcal{L}_{D'})\iota_!(\opn{td}(D))
=(\opn{ch}(\mathcal{L}_{D'})-\opn{ch}(\mathcal{L}_{D'-D}))\opn{td}(X).
\end{align}
...
この論文では、\cref{equation-closed-open-exact} を
\cref{equation-anti-effective-divisor} の一種の
類似と見立てており、
...
\begin{align}
\opn{cyc}_X(D\cdot A)=c_1(\mathcal{L}_{D})
\frown \opn{cyc}_X(A).
\end{align}
...
c_1(\mathcal{L}_{D'})\frown \iota_*[D]
\end{align}

\begin{proof}[{Proof of \cref{proposition-cycle-chern}}]

$D_1,D_2$ は、 sedentarity 0 であると仮定してよい。
...
\end{align}
$D_1,D_2$ は、sedentarity 0 であると仮定したので、
