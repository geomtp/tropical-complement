...
$S$の各点$x\in S$ に対して、コンパクト近傍
$U_x\cap C$ が空ではないことが、$x\in C$と必要十分条件
になるようにとる。$C$は$S$の閉部分集合
なのでそのような取り方は可能である。
さらに、$C_{\mathrm{sing}}$を含む $U_x$ が一つのみになるように
することもできる。
点 $x\in S$ を$\opn{sed}_S(x)=0$ な点とする。
このとき、
\begin{align}
\chi(S\setminus C)=\sum_{\sigma \in I}\opn{sgn}(\sigma)\chi(U_{\sigma}\setminus C)
\end{align}
と書くことができる。ここで、$U_{\sigma}$が$C$の元
を持たないとき、
$\chi_c(U_{\sigma}\setminus C)=\chi(U_{\sigma}\setminus C)$
となる。

\begin{align}
\chi(S\setminus C)-\chi_c(S\setminus C)
=\sum_{x\in C,U_x\in \mathcal{C}}
\chi(U_x\setminus C)-\chi_c(U_x\setminus C)
\end{align}

さらに、$U_x$ は、十分小さい閉球
の$S$への制限に取り換えてもよい。
そうすることで個別に計算を施すことによって、
...
\begin{align}
\chi(S\setminus C)-\chi_c(S\setminus C)
=-\sum_{p\in C_{\mathrm{sing}}}(\opn{val}(p)-2)=-K_S.C
\end{align}

\end{proof}

\begin{remark}
この証明は Euler calculus によって説明できる。
\begin{align}
\chi(S\setminus C)=\chi(Ra_{S*}Rj_*j^{-1}\mathbb{Q}_{S}) 
\end{align}
となる。ここで、次のような $S$ 上の関数を考える。
\begin{align}
\chi(S\setminus C)(x)\deq \chi(Rj_*j^{-1}\mathbb{Q}_{S})_x 
...
の証明と同じ分類より、
\begin{align}
(Rj_*j^{-1}\mathbb{Q}_{S})_x
\simeq
\begin{cases}
\mathbb{Q}[0], \text{ if } x\in S\setminus C \\
\mathbb{Q}^3[0] \text{ if } C_{\mathrm{sing}}
\cap S_{\mathrm{reg}} \\
\mathbb{Q}^2[0] \text{ if otherwise}.  
\end{cases}  
\end{align}
となっている。
\end{remark}
